...
describe('asset registry', () => {
    it('deduplicates identical bytes under different names into one file', () => {
        ...
        expect(registry.files[0].path).toBe('public/assets/images/hero.png');
        // Every source URL resolves to the same physical file.
        for (const src of [
            'https://cdn.test/hero.png',
            'https://cdn.test/hero-copy.png',
            'https://cdn.test/hero-final.png',
        ]) {
            expect(registry.pathBySrc.get(src)).toBe('public/assets/images/hero.png');
        }
    });
    ...
    it('keeps distinct bytes even under the same name (collision-safe suffix)', () => {
        ...
        expect(paths).toContain('public/assets/images/photo.png');
        expect(paths).toContain('public/assets/images/photo-2.png');
        expect(registry.pathBySrc.get('https://cdn.test/a.png')).toBe('public/assets/images/photo.png');
        expect(registry.pathBySrc.get('https://cdn.test/b.png')).toBe('public/assets/images/photo-2.png');
    });

    it('derives deterministic names from URLs for unnamed assets', () => {
        const doc = documentWithAssets([
            asset({
                src: 'https://cdn.test/icons/arrow-right.svg',
                type: 'svg',
        ...
        expect(registry.files[0].path).toBe('public/assets/images/arrow-right.svg');
    });

    it('preserves asset types via mimeType and extension', () => {
        const doc = documentWithAssets([
            asset({
                src: 'https://cdn.test/video',
                type: 'video',
                name: 'demo',
                data: new TextEncoder().encode('mp4'),
                mimeType: 'video/mp4',
            }),
            asset({
                src: 'https://cdn.test/font.woff2',
                type: 'font',
                name: 'Inter',
                data: new TextEncoder().encode('font'),
                mimeType: 'font/woff2',
            }),
            asset({
                src: 'https://cdn.test/pic',
                type: 'image',
                name: 'pic',
        ...
        expect(registry.files.map((file) => file.path)).toContain('public/assets/videos/demo.mp4');
        expect(registry.files.map((file) => file.path)).toContain('public/fonts/Inter.woff2');
        expect(registry.files.map((file) => file.path)).toContain('public/assets/images/pic.png');
    });
    ...
    it('decodes data URIs into real files', () => {
        ...
        expect(registry.uniqueCount).toBe(1);
        expect(registry.files[0].path).toBe('public/assets/images/pixel.png');
        expect(registry.files[0].data?.byteLength).toBeGreaterThan(0);
        expect(registry.pathBySrc.get(dataUri)).toBe('public/assets/images/pixel.png');
    });
    ...
    it('writes every referenced asset into the generated project', async () => {
        ...
        // The image file is written under public/ so Vite ships it in dist/.
        expect(
            findFile({ name: 'AssetsDemo', files: result.files, nodes: result.nodes }, 'public/assets/images/hero.png'),
        ).toBeDefined();

        // The section references the LOCAL path (absolute public URL, served
        // at the root in dev and copied into dist/), never the remote URL.
    ...
    it('warns for fonts with no downloadable source instead of substituting', () => {
        ...
        expect(
            fontProject.warnings?.some(
                (w) =>
                    w.stage === 'fonts' &&
                    w.message.includes('Custom Display') &&
                    w.message.includes('no downloadable file'),
            ),
        ).toBe(true);
    });

    it('does not import the tokens module for a frame with an image fill', () => {
        // An image fill renders as `backgroundImage: url(...)` — never a
        // `colors.*` token reference. Importing the tokens module anyway would
        // leave an unused import that fails the generated project's strict
        // build (TS6133).
            ...
            nodes: [
                {
                    ...
                    style: {
                        fills: [{ type: 'image', image: { src: 'assets/images/banner.png', name: 'banner' } }],
                    },
        ...
        const section = findFile(project, 'src/sections/Banner.tsx');
        expect(section).toBeDefined();
        expect(section!.content).toContain("backgroundImage: `url('assets/images/banner.png')`");
        expect(section!.content).not.toContain('import { colors }');
        expect(section!.content).not.toContain("from '../tokens'");
    });
});
